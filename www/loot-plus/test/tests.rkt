...
(define (make-test-suite name run-proc)
  (test-suite
   ...
   (test-suite
    ...
    (test-suite
     ...
     (check-equal? (run-proc '(define (f x) x)
                             '(let ((y 2))
                                (f 1 y)))
                   'err)
     (check-equal? (run-proc '(define (f x) (g x x))
                             '(define (g y) 1)
                             '(f 1))
                   'err)
     (check-equal? (run-proc '(define (f x) (let ((y x)) (f y y)))
                             '(f 1))
                   'err)
     (check-equal? (run-proc '(define (f) 1)
                             '(f 2))
                   'err)
     (check-equal? (run-proc '(define (f x) (g))
                             '(define (g y) 1)
                             '(f 1))
                   'err)
     (check-equal? (run-proc '(define (f x) (f 1 1))
                             '(f 1))
                   'err)
     (check-equal? (run-proc '(define (f x) 1)
                             '(let ((y 2))
                                (if (zero? y)
                                    (f 7)
                                    (f 3 4))))
    ...
    (test-suite
     ...
     (check-equal? (run-proc '(define (f . x) x)
                             '(let ((y 2))
                                (f 1 2 3)))
                   '(1 2 3))
     (check-equal? (run-proc '(define (f x y . z) y)
                             '(let ((y 2))
                                (f 1 2)))
                   2)
     (check-equal? (run-proc '(define (f x y . z) y)
    ...
    (test-suite
     "Rest arity errors"
     (check-equal? (run-proc '(define (f x . r) x)
                             '(f))
                   'err)
     (check-equal? (run-proc '(define (f x y . r) 1)
                             '(f 2))
                   'err)
     (check-equal? (run-proc '(define (f x y z . r) x)
                             '(let ((y 2))
                                (f 1 y)))
                   'err)
     (check-equal? (run-proc '(define (f x) (let ((y x)) (f y y)))
     ...
     (check-equal? (run-proc '(define (f x) (g x x))
                             ...
                             '(f 1))
                   'err)
     (check-equal? (run-proc '(define (f x y . z) (g))
                             '(define (g y) 1)
                             '(f 1))
                   'err)
     (check-equal? (run-proc '(define (f x y z . r) 1)
                             '(let ((y 2))
                                (if (zero? y)
                                    (f 7 8 9)
                                    (f 3 4))))
    ...
    (test-suite
     ...
     (check-equal? (run-proc '(define (f) 1)
                             '(apply f '()))
                   1)
     (check-equal? (run-proc '(define (f x y z) z)
                             '(apply f (cons 1 (cons 2 (cons 3 '())))))
                   3)
     (check-equal? (run-proc '(define (f x y z) z)
                             '(add1 (apply f (cons 1 (cons 2 (cons 3 '()))))))
                   4)
     (check-equal? (run-proc '(define (f x y z) z)
                             '(let ((z 7)) (add1 (apply f (cons 1 (cons 2 (cons z '())))))))
                   8)
     (check-equal? (run-proc '(define (f x y z) z)
                             '(let ((ls (cons 1 (cons 2 (cons 3 '())))))
                                (add1 (apply f ls))))
                   4)
     (check-equal? (run-proc '(define (f p q x y z) p)
                             '(let ((ls (cons 1 (cons 2 (cons 3 '())))))
                                (apply f #\a #\b ls)))
                   #\a))
