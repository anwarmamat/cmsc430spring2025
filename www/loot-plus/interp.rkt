...
(define (interp-with-handlers ps hs e r ds)
  (match* (ps hs)
    [('() '()) (interp-env e r ds)]
    [((cons p ps) (cons h hs))
     (match (interp-env p r ds)
       ['err 'err]
       [(? procedure? p)
        (match (interp-env h r ds)
          ['err 'err]
          [(? procedure? h)
           ;; We intercept and re-raise Racket errors to prevent accidental
           ;; capture by Loot+ exception handling.
           (with-handlers ([exn? (λ (e) (raise e))]
                           [p h])
             (interp-with-handlers ps hs e r ds))])])]))
